...
// JD (Job Description) tables
export const jds = table(
  'jds',
  {
  ...
  (table) => [index('idx_jds_user_id').on(table.userId)]
);

export const jdTemplates = table(
  'jd_templates',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
    name: text('name').notNull(),
  ...
  (table) => [
    index('idx_jd_templates_category').on(table.category),
    index('idx_jd_templates_is_public').on(table.isPublic),
  ]
);
